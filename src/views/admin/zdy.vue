<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="keyword" clearable class="filter-item" size="small" style="width: 200px;" placeholder="请输入操作人名称" @keyup.enter.native="handleFind" />
      <el-select v-model="query.requestMethod" clearable placeholder="类型" class="filter-item" size="small" style="width: 130px">
        <el-option v-for="item in queryTypeOptions" :key="item.key" :label="item.requestMethod" :value="item.key" />
      </el-select>
      <el-button class="filter-item" type="primary" icon="el-icon-search" size="small" @click="handleFind">查找</el-button>
      <el-button class="filter-item" size="small" type="primary" icon="el-icon-plus" @click="handleDelete1">删除所有异常日志</el-button>
    </div>
...
import { deleteTenant1 } from '../../api/zdy'
...
export default {
  ...
  data() {
    return {
      ...
      queryTypeOptions: [
        { key: 'POST', requestMethod: 'POST' },
        { key: 'GET', requestMethod: 'GET' },
        { key: 'PUT', requestMethod: 'PUT' }
      ],
      query: {
        requestMethod: ''
      },
  ...
  methods: {
    ...
    logList: function() {
      ...
      params.append('requestMethod', this.query.requestMethod)
    ...
    handleDelete1: function() {
      const that = this
      this.$confirm('此操作将删除所有异常日志, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          deleteTenant1().then(response => {
            if (response.data.code === 200) {
              this.$message({
                type: 'success',
                message: '操作成功'
